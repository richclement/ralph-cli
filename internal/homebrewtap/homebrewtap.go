...
const (
	defaultDescription = "Deterministic outer loop that runs an AI agent until completion"
	defaultLicense     = "MIT"
)
...
var formulaTemplate = template.Must(template.New("formula").Parse(`class {{ .ClassName }} < Formula
  ...
  on_macos do
    if Hardware::CPU.arm?
      url "https://github.com/{{ .SourceRepo }}/releases/download/v#{version}/{{ .BinaryName }}_#{version}_darwin_arm64.tar.gz"
      sha256 "{{ .DarwinARM64.SHA256 }}"
    else
      url "https://github.com/{{ .SourceRepo }}/releases/download/v#{version}/{{ .BinaryName }}_#{version}_darwin_amd64.tar.gz"
  ...
  on_linux do
    if Hardware::CPU.arm?
      url "https://github.com/{{ .SourceRepo }}/releases/download/v#{version}/{{ .BinaryName }}_#{version}_linux_arm64.tar.gz"
      sha256 "{{ .LinuxARM64.SHA256 }}"
    else
      url "https://github.com/{{ .SourceRepo }}/releases/download/v#{version}/{{ .BinaryName }}_#{version}_linux_amd64.tar.gz"
...
func Generate(version string, sourceRepo string, formulaName string, binaryName string, checksums []byte) (RenderResult, error) {
	assets, err := ParseChecksums(checksums)
	if err != nil {
		return RenderResult{}, err
	}

	input, err := BuildFormulaInput(version, sourceRepo, formulaName, binaryName, assets)
	if err != nil {
		return RenderResult{}, err
	}
...
func BuildFormulaInput(version string, sourceRepo string, formulaName string, binaryName string, assets map[string]AssetChecksum) (FormulaInput, error) {
	if version == "" {
	...
	if binaryName == "" {
		return FormulaInput{}, errors.New("binary name is required")
	}

	required := map[string]string{
		"darwin_amd64": fmt.Sprintf("%s_%s_darwin_amd64.tar.gz", binaryName, version),
		"darwin_arm64": fmt.Sprintf("%s_%s_darwin_arm64.tar.gz", binaryName, version),
		"linux_amd64":  fmt.Sprintf("%s_%s_linux_amd64.tar.gz", binaryName, version),
		"linux_arm64":  fmt.Sprintf("%s_%s_linux_arm64.tar.gz", binaryName, version),
	}
