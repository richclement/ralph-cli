...
func TestGenerateGoldenFiles(t *testing.T) {
	...
	result, err := Generate("0.1.0", "richclement/ralph-cli", "ralph-cli", "ralph", []byte(sampleChecksums))
	if err != nil {
...
func TestGenerateMissingAsset(t *testing.T) {
	...
	_, err := Generate("0.1.0", "richclement/ralph-cli", "ralph-cli", "ralph", []byte(strings.ReplaceAll(sampleChecksums, "ralph_0.1.0_linux_arm64.tar.gz", "ralph_0.1.0_linux_arm64-missing.tar.gz")))
	if err == nil {
		t.Fatal("expected error for missing asset")
	}
	if !strings.Contains(err.Error(), `required asset "ralph_0.1.0_linux_arm64.tar.gz" missing`) {
...
func TestParseChecksumsDuplicateAsset(t *testing.T) {
	...
	_, err := ParseChecksums([]byte(strings.Join([]string{
		"111 ralph_0.1.0_darwin_amd64.tar.gz",
		"222 ralph_0.1.0_darwin_amd64.tar.gz",
	}, "\n")))
	if err == nil {
		t.Fatal("expected duplicate asset error")
	}
	if !strings.Contains(err.Error(), `duplicate asset "ralph_0.1.0_darwin_amd64.tar.gz"`) {
		t.Fatalf("unexpected error: %v", err)
	}
}
...
func TestBuildFormulaInputIgnoresWindowsAssets(t *testing.T) {
	...
	input, err := BuildFormulaInput("0.1.0", "richclement/ralph-cli", "ralph-cli", "ralph", assets)
	if err != nil {
		t.Fatalf("BuildFormulaInput returned error: %v", err)
	}

	if input.ClassName != "RalphCli" {
		t.Fatalf("unexpected class name: %s", input.ClassName)
	}
	if input.DarwinAMD64.Name != "ralph_0.1.0_darwin_amd64.tar.gz" {
		t.Fatalf("unexpected darwin amd64 asset: %+v", input.DarwinAMD64)
	}
	if input.LinuxARM64.Name != "ralph_0.1.0_linux_arm64.tar.gz" {
...
func TestWriteFiles(t *testing.T) {
	...
	prBodyPath := filepath.Join(dir, "artifacts", "ralph-cli-homebrew-pr-body.md")
	if err := WriteFiles(WriteOptions{
		TapDir:     filepath.Join(dir, "tap"),
		PRBodyPath: prBodyPath,
	}, "ralph-cli", result); err != nil {
		t.Fatalf("WriteFiles returned error: %v", err)
	}

	formulaPath := filepath.Join(dir, "tap", "Formula", "ralph-cli.rb")
...
const sampleChecksums = `1111111111111111111111111111111111111111111111111111111111111111 ralph_0.1.0_darwin_amd64.tar.gz
2222222222222222222222222222222222222222222222222222222222222222 ralph_0.1.0_darwin_arm64.tar.gz
3333333333333333333333333333333333333333333333333333333333333333 ralph_0.1.0_linux_amd64.tar.gz
4444444444444444444444444444444444444444444444444444444444444444 ralph_0.1.0_linux_arm64.tar.gz
5555555555555555555555555555555555555555555555555555555555555555 ralph_0.1.0_windows_amd64.zip`
