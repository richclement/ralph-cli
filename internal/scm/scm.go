...
func (r *Runner) runCommit(ctx context.Context) error {
	r.log("Getting commit message from agent")

	// Get commit message from agent
	output, err := r.AgentRunner.Run(ctx, commitMessagePrompt)
	if err != nil {
...
func extractCommitMessage(output string) string {
	...
	if response, found := loop.ExtractResponse(output); found {
